...
```text
รับอินพุตมาสองสี แล้วเอาคำมาต่อกันด้วยการ+สตริง แบบcol1+col2 แล้วเอามาเช็คเงื่อนไข แต่อย่างแรกต้องให้จัดการกรณีErrorก่อน โดยเอาไว้อันแรกเลยเพื่อไม่ให้โค้ดพังหรือรันต่อจนครบก่อน โดยif col1 not in ("Red","Yellow","Blue")อย่างงี้ แล้วprint("Error") และต่อด้วยif col1+col2 in ("RedYellow","YellowRed") ถ้าตรงเงื่อนไขก็printสีที่ควรได้คือOrange รันไปเรื่อยๆจนครบทุกแบบ แล้วจะเจอเงื่อนไขที่ว่าถ้าสองสีเป็นสีเดียวกันจะprintสีแรกออกมา เช่น Red , Red จะเข้าเงื่อนไขที่ว่าif col1==col2 ให้ทำการprint(col1)ซึ่งคือสีแรกออกมา
```
...
```text
Step 1:ให้รับอินพุตมาสองสี
Step 2:เอาคำมาต่อกันแบบ RedBlue/BlueRed แล้วค่อยให้เข้าเงื่อนไข
Step 3:ถ้าเข้าเงื่อนไขก็printออกมาตามสีที่ได้หรือError
```
...
```text
เหมือนแผนแรก แค่เพิ่มเงื่อนไขที่เมื่อสีซ้ำให้printสีแรกออกมา เพราะเข้าใจเนื้อหาและโจทย์ถูกแล้ว และส่วนนึงของโค้ดก็ใช้ได้แล้ว แค่ยังไม่ครอบคลุมทุกกรณี
```
...
```text
มีสีซ้ำให้เข้าเงื่อนไขกรณีสีซ้ำ
```
...
```text
ช่วยกันหาวิธีแก้โค้ดและช่วยกันดูtest case /// ช่วยด็เรื่องtest case
```

คุณยังทำอะไรด้วยตนเอง

```text
เขียนโค้ด และนั่งคิดกรณีต่างๆ
```
